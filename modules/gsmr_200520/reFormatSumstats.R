...
sumstat.fn="data/HERMES_Jan2019_HeartFailure_summary_data.txt"
...
sumstat.fn="data/nielsen-thorolfsdottir-willer-NG2018-AFib-gwas-summary-statistics.tbl.gz"
...
gctaSumstat = data.frame(SNP= sumstat$rs_dbSNP147,
                         A1 = toupper(sumstat$A2),
                         A2 = toupper(sumstat$A1),
                         freq = sumstat$Freq_A2,
                         b = sumstat$Effect_A2,
                         se = sumstat$StdErr,
                         p = sumstat$Pvalue,
                         N = 1030836,
...
#clean
gctaSumstat = gctaSumstat[-which(is.na(gctaSumstat$freq)),]
gctaSumstat = gctaSumstat[gctaSumstat$freq > 0.005 & gctaSumstat$freq < 0.995,]
gctaSumstat = gctaSumstat[!is.na(gctaSumstat$SNP),]

sum(grepl("rs",gctaSumstat$SNP))

gctaSumstat = gctaSumstat[grepl("rs",gctaSumstat$SNP),]


idxDup = which(duplicated(gctaSumstat$SNP))
gctaSumstat$SNP[idxDup] = paste0(gctaSumstat$SNP[idxDup],".2")

# round 2
idxDup = which(duplicated(gctaSumstat$SNP))
idxRM = which(gctaSumstat$SNP %in% gctaSumstat$SNP[idxDup] )
gctaSumstat = gctaSumstat[-idxRM,]
...
stroke.fn = c("data/MEGASTROKE.1.AS.EUR.out",
              "data/MEGASTROKE.2.AIS.EUR.out",
              "data/MEGASTROKE.4.CES.EUR.out")
...
la.list = c("lamin.bgen.stats.gz",
            "lamax.bgen.stats.gz",
            "ilamin.bgen.stats.gz",
            "ilamax.bgen.stats.gz",
            "laaef.bgen.stats.gz",
            "lapef.bgen.stats.gz",
            "latef.bgen.stats.gz")

la.list = paste0("../gwas_w_bolt_v2_200420/results/gwas_regular/", la.list)
...
for(i in 1:length(la.list)) {
    ...
    #out.fn = paste0("data/",gsub(".bgen.stats.gz","",basename(sumstat.fn)),".mixed.gcta.tsv")
    ...
    #sumstat2.fn = rntrn_la.list[i]
    #sumstat2 = fread(sumstat2.fn,
    #                stringsAsFactors = F,
    #                header = T
    #                )
