locus.plot <- function(snp, locusname, chr, locus, range, best.pval,
                       genelist) {
  ...
  recomb <- read.table(paste("data/genetic_map_HapMapII_GRCh37/genetic_map_GRCh37_chr", chr, ".txt", sep=""), header=T)
  ...
  genes.in.locus <- subset(genelist, 
                           ( genelist$START > min.pos & genelist$START < max.pos ) | ( genelist$END > min.pos & genelist$END < max.pos) 
                           )
