...
pthAF=${DIR}/results/rntrn_testCovar
...
for sumstat in ${sumstats[@]}; do
    ...
    gzcat ${pthAF}/$sumstat | grep -F -f ${pheno}_leadSnps.txt - > results/${pheno}_testCovar_leadSnps.txt
    cat results/${pheno}_testCovar_leadSnps.txt | perl -ane 'if($F[15] <= 5e-8){print $_}' > tmp
    mv tmp results/${pheno}_testCovar_leadSnps.txt
